...
def down_bd():
    ...
    for row in rows:
        ...
        conn.close()
    return history
def experience1(dfs, date, currency, fig):# ЧИСТЫЙ ДООХОД сделать суммируюущую переменную с добавками от курсов!
    ...
    hystory = {
            'KRUR':
                    {
                         date[0]:{'BTC':100},#'BTC':100
                         date[2]:{'add': 3000},
                        #  date[2]:{'+': 2000},
                        #  date[3]:{'-': 3000},
                        #  date[4]:{'+': 3000},
                         date[6]:{'withdraw': 1000, '+':300},# если захочешь повторить одну и ту же операцию то просто сложение делать по ключу!!! 
                         #можно просто перед строительством графика пройтись циклом по всем словарям и проссумировать операции
                         date[8]:{'withdraw': 1000},
            ...
            try:
                ...
                if  list(hystory[j][date[i]].keys())[0] == '+':
                    ...
                    fig.add_trace(go.Scatter(x=[date[i],date[i]] ,
                                         ...
                                         y = [port[j], (port[j] + hystory[j][date[i]]['+'])],
...
experience1(dfs, date[:30], option, fig)
