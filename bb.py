    ...
    for i in range(len(date)-1):
        ...
        for j in list(cur_port.keys()):
            ...
            try:
                ...
                try:
                     if date[i] in list(hystory[j].keys())   and j  in list(hystory[j][date[i]].keys()):
                        listic.append(j)
                except:
                     pass
                if i!=len(date)-1 and j not in listic:# and date[i] in list(hystory[j].keys())   and i not in list(hystory[j][date[i]].keys()):
